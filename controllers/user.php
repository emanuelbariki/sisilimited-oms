<?
	...
	if ($action == 'ajax_updatetheme') {
		// debug($_GET);
		$color = $_GET['color'];
		$id = $_SESSION['member']['id'];
		$Users->update($id, array('themecolor' => $color));
		die();
	}
	if ( $action == 'inactive' ) {
	...
	if ( $action == 'users_save' ) {
		...
		if ( empty($id) )  {
			...
			if (!empty($_FILES['image'])) {
				$image = $_FILES['image'];
				$img = resizeUploadImage($image, $refwidth = 1200, $refheight = 0, 'assets/images/users/', $format = 'jpg');
			...
			redirect('users','users_rights','id='.$id);
		}else{
